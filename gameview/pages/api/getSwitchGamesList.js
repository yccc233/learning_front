...
export default async function getSwitchGamesList(req, res) {
    ...
    async function fetchSwitchDataChunk(page) {
        try {
    ...
    async function getTotalPagesIn520Switch() {
        try {
    ...
    async function* dataStream() {
        ...
        const totalPage = await getTotalPagesIn520Switch()
        ...
        while (page <= totalPage) {
            const dataChunk = JSON.stringify({
                type: "list",
                data: await fetchSwitchDataChunk(page),
                page: page
            });
